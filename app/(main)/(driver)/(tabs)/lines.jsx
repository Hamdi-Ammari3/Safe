...
import noLines from '../../../../assets/images/nothing.png'
  ...
  const startTodayJourney = async () => {
    try {
      ...
      if (!existingTracking[yearMonthKey][dayKey]) {
        existingTracking[yearMonthKey][dayKey] = { start_the_journey: false };
      }
      ...
      const todayTrackingLines = activeLinesToday.map((line) => {
        const todaySchedule = line.timeTable.find(d => d.dayIndex === todayIndex);
        const startTime = todaySchedule?.startTime ? todaySchedule.startTime: null;
        const endTime = todaySchedule?.endTime ? todaySchedule.endTime : null
    ...
    return(
      <SafeAreaView style={styles.container}>
        <View style={styles.add_your_data_container}>
          ...
          <View style={styles.illustration_container}>
            <Image source={noLines} style={styles.illustration}/>
          </View>
          <View>
