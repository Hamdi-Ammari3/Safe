...
const LineWithoutDriver = ({rider}) => {
    ...
    // Calculate line subs fee amount
    const formatTotalSubscriptionFee = (line) => {
        const isInstitutionRider = institutions.some(inst => inst.name === line.destination)

        if ( 
            !rider || 
            !rider.line_id || 
            !rider.driver_commission || 
            !rider.company_commission || 
            isInstitutionRider
        ) return "--";

        const total = rider.driver_commission + rider.company_commission;
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.line_text_box}>
                <Text style={styles.line_text}>الاشتراك الشهري: {formatTotalSubscriptionFee(line)}</Text>
            </View>
            <View style={styles.line_text_box}>
